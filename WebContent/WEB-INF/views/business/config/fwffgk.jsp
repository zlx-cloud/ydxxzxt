...
<script type="text/javascript">
	...
	function openAddView() {
		...
		$("#addUpdate").dialog("open").dialog("setTitle", "添加服务方法管控");
		url = "${ctx}/configServiceManage/add?flag=1";
	}
	...
	function deleteConfig() {
		...
		$.messager.confirm("系统提示", "您确认要删除这条数据吗？", function(r) {
			if (r) {
				$.post("${ctx}/configServiceManage/delete", {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function updateStatus(id, enabled, yybs, fwbs) {
		$.post("${ctx}/configServiceManage/updateStatus", {
			id : id,
			enabled : enabled,
			yybs : yybs,
			fwbs : fwbs
...
<body class="easyui-layout">
	<div data-options="region:'center'">
		<table id="dg" class="easyui-datagrid" fitColumns="true" singleSelect="true"
			pagination="true" rownumbers="true" url="${ctx}/configServiceManage/list" fit="true" toolbar="#tb">
			<thead>
				<tr>
					...
					<th field="concurrency" align="center">服务并发数</th>
					<th field="waitTime" align="center">等待时间</th>
					<th field="serviceLevel" align="center">服务级别</th>
		...
		<div id="addUpdate" class="easyui-dialog" style="width: 620px; height: 200px;
			...
			<form id="addUpdatefm" method="post">
				<table cellspacing="5px;">
					...
					<tr>
						<td>服务并发数：</td>
						<td>
							<input type="text" id="concurrency" name="concurrency" class="easyui-textbox"/>
						</td>
						<td>等待时间：</td>
						<td>
							<input type="text" id="waitTime" name="waitTime" class="easyui-textbox"/>
						</td>
					</tr>
					<tr>
						<td>服务级别：</td>
						<td>
							<select class="easyui-combobox" name="serviceLevel" id="serviceLevel" style="width:100%;">
