...
<script type="text/javascript">
	...
	function openAddView() {
		...
		$("#addUpdate").dialog("open").dialog("setTitle", "添加访问超时配置");
		url = "${ctx}/configAccessTimeout/add?flag=1";
	}
	...
	function deleteConfig() {
		...
		$.messager.confirm("系统提示", "您确认要删除这条数据吗？", function(r) {
			if (r) {
				$.post("${ctx}/configAccessTimeout/delete", {
					id : id
				}, function(result) {
					if (result.success) {
	...
	function updateStatus(id, enabled, yybs, fwbs) {
		$.post("${ctx}/configAccessTimeout/updateStatus", {
			id : id,
			enabled : enabled,
			yybs : yybs,
			fwbs : fwbs
...
<body class="easyui-layout">
	<div data-options="region:'center'">
		<table id="dg" class="easyui-datagrid" fitColumns="true" singleSelect="true"
			pagination="true" rownumbers="true" url="${ctx}/configAccessTimeout/list" fit="true" toolbar="#tb">
			<thead>
				<tr>
					...
					<th field="timeout" align="center">超时时间（秒）</th>
			...
			<form id="addUpdatefm" method="post">
				<table cellspacing="5px;">
					...
					<tr>
						<td>超时时间（秒）：</td>
						<td>
							<input type="text" id="timeout" name="timeout" class="easyui-textbox"/>
						</td>
						<td>处置策略：</td>
						<td>
