...
<%-- <script type="text/javascript" src="${ctx}/static/easyui/datagrid-detailview.js"></script> --%>
	...
	function searchFwzyzcb(){
		...
		$('#dg').datagrid('load',{
			fwbs:$("#fwbsindex").val(),
			ffbs:$("#ffbsindex").val(),
			qqcsbs:$("#qqcsbsindex").val(),
			fwmc:$("#fwmcindex").val(),
			ffmc:$("#ffmcindex").val(),
			qqcsm:$("#qqcsmindex").val()
		});
	}
   ...
   function openUploadDialog(){
 		$("#dlg").dialog("open").dialog("setTitle","服务方法请求参数注册信息");
 	...
 	function openFwzcModifyDialog(){
		...
		$("#addUpdate").dialog("open").dialog("setTitle","修改服务方法请求参数信息");
		$("#fwmc").textbox('textbox').css('background','#E0ECFF');
		$('#qqcsbs').textbox('textbox').attr('readonly',true); 
		$("#qqcsbs").textbox('textbox').css('background','#E0ECFF');
	...
	function openFwzcAddDialog(){
		...
		$('#qqcsbs').textbox('textbox').attr('readonly',false); 
		$("#qqcsbs").textbox('textbox').css('background','');
		$("#fwmc").textbox('textbox').css('background','#E0ECFF');
		$("#addUpdate").dialog("open").dialog("setTitle","添加服务方法请求参数信息");
		
		url="${ctx}/fwFFqqcsManage/add?flag=1";
	}
	...
	function deleteFwff(){
		...
		var qqcsbs=selectedRows[0].qqcsbs;
		$.messager.confirm("系统提示","您确认要删除这条数据吗？",function(r){
			if(r){
				$.post("${ctx}/fwFFqqcsManage/delete",{fwbs:fwbs,ffbs:ffbs,qqcsbs:qqcsbs},function(result){
					if(result.success){
...
//url：窗口调用地址，title：窗口标题，width：宽度，height：高度，shadow：是否显示背景阴影罩层  ${ctx}/fwSqhisManage/show
	function showMessageDialog() {  
		...
		var url="${ctx}/fwFFqqcsManage/showHis?fwbs="+row.fwbs;
...
<body class="easyui-layout" >
        ...
        <div data-options="region:'center'">
          <table id="dg"  class="easyui-datagrid" fitColumns="true"  singleSelect="true" pagination="true" rownumbers="true" url="${ctx}/fwFFqqcsManage/list" fit="true" toolbar="#tb">
                <thead>
                    <tr>
    		     ...
    		     <th field="ffmc"  align="center">方法名称</th>
    		      <th field="qqcsbs"  align="center">请求参数标识</th>
    		      <th field="qqcsm"  align="center">请求参数名称</th>
    		      <th field="qqcslx"  align="center">请求参数类型</th>
    		       <th field="qqcscd"  align="center">请求参数长度</th>
    		       <th field="qqcszy"  align="center">请求参数值域</th>
                    </tr>
                </thead>
            </table>
       <div id="tb">
	<div>
		...
		<a href="${ctx}/fwFFqqcsManage/exportExcel" class="easyui-linkbutton" iconCls="icon-download-chart" plain="true">下载服务</a>
	...
	<div>
		...
		&nbsp; 请求参数标识：&nbsp;<input type="text" class="easyui-textbox"  name="qqcsbsindex" id="qqcsbsindex" size="20" /></br>
		...
		&nbsp; 方法名称：&nbsp;<input type="text" class="easyui-textbox"  name="ffmcindex" id="ffmcindex" size="20" />
       	&nbsp; 请求参数名称：&nbsp;<input type="text" class="easyui-textbox"  name="qqcsmindex" id="qqcsmindex" size="20" />
...
<div id="dlg" class="easyui-dialog" style="width:100%;max-width:400px;padding:10px 60px;"
  ...
  <form id="fm" method="post" enctype="multipart/form-data">
		<div style="margin-bottom:20px">
			<input class="easyui-filebox" id="fileFwqqcs" name="fileFwqqcs" label="服务方法请求参数注册:" labelPosition="top" data-options="buttonText:'选择文件',prompt:'选择文件...'" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel" style="width:100%"><a href="${ctx}/uploadServe/downLoadExcelModel?flag=fwqqcs">模板下载</a>
		</div>
			...
			<!-- <input class="easyui-filebox" type="hidden" id="fileFwqqcs" name="fileFwqqcs" /> -->
...
<div id="addUpdate" class="easyui-dialog" style="width: 620px;height: 250px;padding: 10px 20px" closable="false"
  ...
  <form id="addUpdatefm" method="post">
  	...
  	<tr>
  		...
  		<tr>
  		     <td>请求参数标识：</td>
  			<td><input type="text" id="qqcsbs" name="qqcsbs" class="easyui-textbox" required="true"/></td>
  			<td>请求参数名称：</td>
  			<td><input type="text" id="qqcsm" name="qqcsm" class="easyui-textbox" required="true"/></td>
  		</tr>
  		<tr>
  		   <td>请求参数类型：</td>
  			<td><input type="text" id="qqcslx" name="qqcslx" class="easyui-textbox" required="true" value="String"/></td>
  			<td>请求参数长度：</td>
  			<td><input type="text" id="qqcscd" name="qqcscd" class="easyui-textbox" required="true"/></td>
  		</tr>
  		<tr>
  		   <td>请求参数值域：</td>
  			<td><input type="text" id="qqcszy" name="qqcszy" class="easyui-textbox" /></td>
  			<td></td>
  			<td></td>
  		</tr>
  	</table>
