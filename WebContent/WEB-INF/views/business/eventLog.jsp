...
<script type="text/javascript">
	function searchVarSystem(){
		var startTime = $("#startTimeSearch").val().substring(0,10);
		var endTime =$("#endTimeSearch").val().substring(0,10);
		if(startTime!=endTime){
			$.messager.alert('系统提示','开始日期和结束日期必须是同一天');
			return;
		}
		$('#dg').datagrid('options').url='${ctx}/eventLog/list';
		$('#dg').datagrid('load',{
			qqbwbsSearch:$("#qqbwbsSearch").val(),
// 			fwmcfwcyfmcSearch:$("#fwmcfwcyfmcSearch").val(),
			fwbsSearch:$("#fwbsSearch").val(),
			ffmc:$('#ffmcSearch').combobox('getValue'),
...
<table id="dg"  class="easyui-datagrid" fitColumns="true" 
    pagination="true" rownumbers="true" url="${ctx}/eventLog/list?startTimeSearch=${startTime}&endTimeSearch=${endTime}&fwqqzZcxxSearch=A00100000000000100003" fit="true" toolbar="#tb">
    <thead>
    	<tr>
	...
	<div>
		...
		<select name="fwqqzZcxxSearch" id="fwqqzZcxxSearch" class="easyui-combobox" >
			...
			<c:forEach items="${fwcyf}" var="zcxx" varStatus="index">
				<option value="${zcxx.fwcyfYyxtbh}"
					<c:if test="${zcxx.fwcyfYyxtmc == 'admin'}">
						selected
					</c:if>
				>${zcxx.fwcyfYyxtmc}</option>
			</c:forEach>
		</select>
		&nbsp;服务名称：&nbsp;
		...
		&nbsp;方法名称：&nbsp;
		<select name="ffmcSearch" id="ffmcSearch" class="easyui-combobox" style="width:230px;">
...
<!-- 		&nbsp;模糊搜索：&nbsp;<input type="text" name="fwmcfwcyfmcSearch" id="fwmcfwcyfmcSearch" size="20" /> -->
		...
		<a href="${ctx}/eventLog/eventLogExport" class="easyui-linkbutton" iconCls="icon-download-chart" plain="true">导出</a>
	</div>
</div>
