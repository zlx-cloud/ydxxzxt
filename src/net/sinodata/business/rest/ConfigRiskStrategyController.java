...
@Controller
@RequestMapping(value = "/configRiskStrategy")
public class ConfigRiskStrategyController {

	@Autowired
	private ConfigRiskStrategyService configRiskStrategyService;
	@Autowired(required = false)
	private ConfigInfo configInfo;

	@RequestMapping(value = "/show", method = RequestMethod.GET)
	public String show(Model model) {
		return "business/config/fxclgl";
	}
	...
	public Object list(Page page, HttpServletRequest request, HttpServletResponse response) {
		...
		SearchResult result = configRiskStrategyService.list(page, condition);
		return result;
	}

	@RequestMapping(value = "/add", method = RequestMethod.POST)
	@ResponseBody
	public void addUpdate(ConfigRiskStrategy crs, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		JSONObject result = new JSONObject();
		String flag = request.getParameter("flag");

		String riskMsg = "";
		if ("FAIL".equals(crs.getRiskType())) {
			riskMsg = "0:" + crs.getF0() + ";1:" + crs.getF1() + ";2:" + crs.getF2();
		} else if ("RT".equals(crs.getRiskType())) {
			riskMsg = "0:" + crs.getRt0() + ";1:" + crs.getRt1() + ";2:" + crs.getRt2();
		}
		crs.setRiskMsg(riskMsg);

		int i = 0;
		if (StringUtil.isEmpty(flag)) {
			crs.setUpdateTime(DateUtil.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss"));
			i = configRiskStrategyService.updateByPrimaryKeySelective(crs);
		} else {
			crs.setId(UUIDGeneratorUtil.getUUID());
			crs.setEnabled("N");
			crs.setCreateTime(DateUtil.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss"));
			i = configRiskStrategyService.insertSelective(crs);
		}
		if (i > 0) {
	...
	public void delete(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int delNums = configRiskStrategyService.deleteByPrimaryKey(id);
		if (delNums > 0) {
			result.put("success", true);
	...
	public void updateStatus(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int i = configRiskStrategyService.updateStatus(id, enabled, updateTime);
		JSONObject result = new JSONObject();
		if (i > 0) {
			result.put("success", "true");
