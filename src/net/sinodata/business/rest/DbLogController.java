...
public class DbLogController {
	...
	public Object list(Page page, HttpServletRequest request, HttpServletResponse response)
		...
		request.getSession().setAttribute("dbLogSearchCondition", condition);
		return new SearchResult(list, (int) search.hits().total().value());
	}

	@RequestMapping(value = "dbLogExport")
	public Object dbLogExport(HttpServletRequest request, HttpServletResponse response) {
		// 文件字符输出流
		FileOutputStream toClient = null;
		try {
			Map<String, Object> condition = (Map<String, Object>) request.getSession()
					.getAttribute("dbLogSearchCondition");
