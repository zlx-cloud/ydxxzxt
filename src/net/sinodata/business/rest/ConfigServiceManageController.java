...
@Controller
@RequestMapping(value = "/configServiceManage")
public class ConfigServiceManageController {

	@Autowired
	private ConfigServiceManageService configServiceManageService;
	@Autowired(required = false)
	private ConfigInfo configInfo;

	@RequestMapping(value = "/show", method = RequestMethod.GET)
	public String show(Model model) {
		return "business/config/fwffgk";
	}
	...
	public Object list(Page page, HttpServletRequest request, HttpServletResponse response) {
		...
		SearchResult result = configServiceManageService.list(page, condition);
		return result;
	}

	@RequestMapping(value = "/add", method = RequestMethod.POST)
	@ResponseBody
	public void addUpdate(ConfigServiceManage configServiceManage, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		JSONObject result = new JSONObject();
		String flag = request.getParameter("flag");
		int i = 0;
		if (StringUtil.isEmpty(flag)) {
			configServiceManage.setUpdateTime(DateUtil.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss"));
			i = configServiceManageService.updateByPrimaryKeySelective(configServiceManage);
		} else {
			configServiceManage.setId(UUIDGeneratorUtil.getUUID());
			configServiceManage.setEnabled("N");
			configServiceManage.setCreateTime(DateUtil.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss"));
			i = configServiceManageService.insertSelective(configServiceManage);
		}
		if (i > 0) {
	...
	public void delete(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int delNums = configServiceManageService.deleteByPrimaryKey(id);
		if (delNums > 0) {
			result.put("success", true);
	...
	public void updateStatus(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int i = configServiceManageService.updateStatus(id, enabled, updateTime);
		JSONObject result = new JSONObject();
		if (i > 0) {
			result.put("success", "true");
