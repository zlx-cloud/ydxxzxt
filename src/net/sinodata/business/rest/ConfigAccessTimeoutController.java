...
@Controller
@RequestMapping(value = "/configAccessTimeout")
public class ConfigAccessTimeoutController {
	...
	public Object list(Page page, HttpServletRequest request, HttpServletResponse response) {
		...
		SearchResult result = configAccessTimeoutService.list(page, condition);
		return result;
	}

	@RequestMapping(value = "/add", method = RequestMethod.POST)
	@ResponseBody
	public void addUpdate(ConfigAccessTimeout configAccessTimeout, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		JSONObject result = new JSONObject();
		String flag = request.getParameter("flag");
		int i = 0;
		if (StringUtil.isEmpty(flag)) {
			configAccessTimeout.setUpdateTime(DateUtil.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss"));
			i = configAccessTimeoutService.updateByPrimaryKeySelective(configAccessTimeout);
		} else {
			configAccessTimeout.setId(UUIDGeneratorUtil.getUUID());
			configAccessTimeout.setEnabled("N");
			configAccessTimeout.setCreateTime(DateUtil.formatDate(new Date(), "yyyy-MM-dd HH:mm:ss"));
			i = configAccessTimeoutService.insertSelective(configAccessTimeout);
		}
		if (i > 0) {
	...
	public void delete(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int delNums = configAccessTimeoutService.deleteByPrimaryKey(id);
		if (delNums > 0) {
			result.put("success", true);
